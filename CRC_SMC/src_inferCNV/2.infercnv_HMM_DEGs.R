...
ord <- read.delim('~/annotation/gencodev34/gencode.v34lift37.annotation.geneorder.txt', header = F, row.names = 1)
...
for (pt in colnames(cnv_degs)) {
  ...
  for (deg in pt_degs) {
    ...
    #cnv_degs[deg, pt] <- names(summ[summ == max(summ)])
    cnv_degs[deg, pt] <- mean(as.numeric(names(summ[summ == max(summ)])))
    if (as.numeric(summ[summ == max(summ)]) < min_prop) {
      min_prop <- as.numeric(summ[summ == max(summ)])
    }
...
ggsave("degs_hmmStates/Proportion.pdf", units = "cm", width = 12, height = 4)
