...
newVariGenes <- function(inputObj, outputPath, libName, regress, pcNum = 60, pltf = "3p", base = "log") {
  if(!pltf %in% c("3p", "ss2")) { stop("pltf should be one of '3p' or 'ss2'") }
  if(!base %in% c("log", "log2")) { stop("(*ss2 mode only) base should be one of 'log' or 'log2'") }
  ...
  for (lib in levels(inputObj@meta.data[,libIndex])) {
    ...
    if (pltf == "3p") {
      ...
      if (base == "log") {
        tmp_expr <- exp(tmp_expr)-1
        tmp_expr <- rowMeans(tmp_expr)
        tmp_expr <- log1p(tmp_expr)
      } else if (base == "log2") {
        tmp_expr <- 2**tmp_expr-1
        tmp_expr <- rowMeans(tmp_expr)
        tmp_expr <- log2(tmp_expr +1)
      }
      lib_count[, lib] <- tmp_expr
    }
