...
# 2. clustering
various_res <- function(inputObj, pcNum, scaleFactor, rangeMax = 2) {
  library(Seurat)
  library(ggplot2)
  library(plyr)
  
  dir.create('res_various')
  dir.create('res_various/markers')
  ...
  for (res in c(1:endpoint)){
    ...
    if (length(levels(Idents(inputObj))) != 1) {
      ...
      write.table(markers, paste(c('res_various/markers/markers_', pcNum, 'pcs_.__res_', res, '__.txt'), collapse = ''), sep = '\t', quote = F, col.names = T, row.names = F) 
    }
  }
  ...
  write.table(label_w, 'res_various/labels.txt', sep = '\t', quote = F, col.names = T, row.names = F)
  return(inputObj)
}
