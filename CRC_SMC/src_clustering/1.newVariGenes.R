...
newVariGenes <- function(inputObj, outputPath, libName, regress, pcNum = 60) {
  ...
  VariableFeaturePlot(inputObj)
  ggsave(paste0(c(outputPath, 'stats/2.variableFeatures.new.pdf'), collapse = '/'), units = 'cm', width = 17, height = 10)
  
  
  ### 4. Calculate PCs
  if ( !is.null(regress) ) {
    print (paste(c("regress: ", paste(regress, collapse = ", ")), collapse = ""))
    inputObj <- ScaleData(object = inputObj, vars.to.regress = regress)
  } else {
    inputObj <- ScaleData(object = inputObj)
  }
  inputObj <- RunPCA(object = inputObj, npcs = pcNum)
  inputObj <- JackStraw(object = inputObj, num.replicate = 100, dims = pcNum)
